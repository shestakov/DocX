...
namespace Novacode
{
    ...
    public class DocX : Container, IDisposable
    {
                ...
                else
                {
                    ...
                    switch (remote_pp.ContentType)
                    {
                        ...
                        case "application/vnd.openxmlformats-officedocument.wordprocessingml.styles+xml":
							...
							PartAction(stylesPart, tr => styles = XDocument.Load(tr));
                            break;

                        case "application/vnd.ms-word.stylesWithEffects+xml":
                            stylesWithEffectsPart = packagePart;
							PartAction(stylesWithEffectsPart, tr => stylesWithEffects = XDocument.Load(tr));
                            break;

                        case "application/vnd.openxmlformats-officedocument.wordprocessingml.fontTable+xml":
                            fontTablePart = packagePart;
							PartAction(fontTablePart, tr => fontTable = XDocument.Load(tr));
                            break;

                        case "application/vnd.openxmlformats-officedocument.wordprocessingml.numbering+xml":
                            numberingPart = packagePart;
							PartAction(numberingPart, tr => numbering = XDocument.Load(tr));
                            break;
        ...
        {
			var new_pp = package.CreatePart(pp.Uri, pp.ContentType, CompressionOption.Normal);
        ...
        internal XDocument AddStylesForList()
        {
            ...
            if (!listStyleExists)
            {
				...
				ProcessPackagePart(package.GetPart(wordStylesUri), tw => wordStyles.Save(tw));
        ...
        public void Save()
        {
            ...
            // Save the main document
			ProcessPackagePart(this.PackagePart, tw => mainDoc.Save(tw, SaveOptions.None));
            ...
            if (sectPr != null)
            {
                ...
                if (evenHeaderRef != null)
                {
					...
					ProcessPackagePart(package.GetPart(target), tw =>
					{
						new XDocument
						(
				...
				ProcessPackagePart(settingsPart, tw => settings.Save(tw, SaveOptions.None));
                if (endnotesPart != null)
                {
					ProcessPackagePart(endnotesPart, tw => endnotes.Save(tw, SaveOptions.None));
				}

                if (footnotesPart != null)
                {
					ProcessPackagePart(footnotesPart, tw => footnotes.Save(tw, SaveOptions.None));
				}

                if (stylesPart != null)
                {
					ProcessPackagePart(stylesPart, tw => styles.Save(tw, SaveOptions.None));
				}

                if (stylesWithEffectsPart != null)
                {
					ProcessPackagePart(stylesWithEffectsPart, tw => stylesWithEffects.Save(tw, SaveOptions.None));
				}

                if (numberingPart != null)
                {
					ProcessPackagePart(numberingPart, tw => numbering.Save(tw, SaveOptions.None));
				}

                if (fontTablePart != null)
                {
					ProcessPackagePart(fontTablePart, tw => fontTable.Save(tw, SaveOptions.None));
				}
            }
